...
dynamodb_table = dynamodb.Table('student_records')
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if http_method == 'GET' and path == student_path:
            ...
            response = modify_student(body['studentId'], body['updateKey'], body['updateValue'])
        elif http_method == 'DELETE' and path == student_path:
            body = json.loads(event['body'])
            response = delete_student(body['studentId'])
        else:
